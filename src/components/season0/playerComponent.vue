...
const entry: LeaderboardEntry = dialogRef.value.data.entry
const totalPlayers: number = dialogRef.value.data.totalPlayers
...
<template>
  <div class="flex flex-row h-96 gap-8 p-4">
    ...
    <div class="flex align-middle h-full w-40 relative flex-shrink-0">
      ...
      <img
        :src="getBodyUrl(entry.playerUuid)"
        alt="player image"
    ...
    <div class="flex flex-col flex-grow gap-4 justify-between w-fit sm:w-[300px]">
      ...
      >
        ...
        <div class="text-4xl font-bold">{{ entry.score.toLocaleString() }}</div>
      </div>
      ...
      <div class="space-y-2 flex-grow">
        ...
        >
          ...
          <span class="text-lg font-semibold">{{ entry.playerName }}</span>
        </div>
        ...
        <div class="flex justify-between flex-col items-start sm:flex-row sm:items-center sm:gap-2">
          ...
          <div class="flex items-center gap-2 text-nowrap">
            <span v-if="getMedalEmoji(entry.rank)" class="text-2xl">
              {{ getMedalEmoji(entry.rank) }}
            </span>
            <span class="font-bold text-blue-600">
              {{ entry.rank }}
              <span class="text-gray-500"> / {{ totalPlayers }} </span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="border-t pt-3">
        ...
        <div class="flex gap-2 flex-wrap">
          <Tag v-if="entry.rank === 1" value="First Place (SL-0)" pt:root="drop-shadow-sm" />
          <Tag v-else-if="entry.rank === 2" value="Second Place (SL-0)" pt:root="drop-shadow-sm" />
          <Tag v-else-if="entry.rank === 3" value="Third Place (SL-0)" pt:root="drop-shadow-sm" />
